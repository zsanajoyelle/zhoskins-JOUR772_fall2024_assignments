...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer: There are 24 rows.**
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer: It shows the population of each race as well as those over 18 for each geocode.**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer: Prince George's County has the highest Black 18+ population and the highest percentage of Democrats.**
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer: The ZCTA5N column's datatype is numerical but it should be chr.**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer: Now the ZCTA5N datatype is chr.**
...
```{r}
tax_assessments <- read.csv('data/assessment.csv') 

maryland_zcta_assessment <- maryland_zcta |> left_join(tax_assessments, join_by(ZCTA5N==zipcode))
```
