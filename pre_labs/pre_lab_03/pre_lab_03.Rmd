...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer: The "Vendor Name" field is chr. The "Amount field is numerical. The Date column shows dates and times.**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer: Some people abbreviated instead of writing out "Loan" or "Grant". And there are also some years that say "N/A", which suggests that the state of MD didn't give loans or grants this year. But it is unclear.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer: A Grant from the Department of Natural Resources/Rural Legacy Program to the Land Preservation Trust, Inc. for \$60 on June 30, 2009. The grant was for Catherine & Mary Gemmill Easement.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer: It takes out capitals and spaces and cleans the header names, then creates a new spreadsheet.**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer: The new dataframe has 17,740 rows.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer: The new zip5 column now only shows zip codes with 5 digits.**
...
21202 received the most amount of money from the Maryland Tourism Board with a total amount of \$648,069. This makes sense because 21202 is Baltimore, which is the biggest city and metro area in the state.
