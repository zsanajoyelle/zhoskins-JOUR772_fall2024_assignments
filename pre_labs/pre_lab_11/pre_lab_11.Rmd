...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer: The unique function shows the unique words in an original list.**
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer: It converts all the text to lowercase and reverts all punctuations so that R can better understand the text.**
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer: "Critical" being negative while "Important" being positive seems a bit counter-intuitive because they can both be used in the same context depending on the statement. "Like" is also positive but it can be used for similes rather than saying you enjoy something. So that also would depend on the statement. "Significant" could be used in positive or negative connotation. Overall, the sentiments seem to be accurate, but there are some words that either could be used in different connotations or may need more context to determine their sentiment.**
